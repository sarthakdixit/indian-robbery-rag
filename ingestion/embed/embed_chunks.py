...
def embed_misses(
    ...
    for batch_index in range(total_batches):
        ...
        if isinstance(result, QuotaExhausted):
            logger.warning(
                "  QUOTA EXHAUSTED at batch %d/%d. retry_after=%s. Stopping.",
                batch_index + 1, total_batches,
                f"{result.retry_after_seconds}s" if result.retry_after_seconds else "unknown",
            )
            logger.warning(
...
def main() -> int:
    ...
    with EmbeddingCache(cache_path) as cache:
        ...
        succeeded, failed, quota_signal = embed_misses(client, cache, misses)
